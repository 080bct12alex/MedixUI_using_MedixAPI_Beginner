...
export default function CreatePatient() {
  ...
  const handleAddDiagnosis = () => {
    setFormData({
      ...formData,
      diagnoses_history: [...formData.diagnoses_history, currentDiagnosis],
    });
    setCurrentDiagnosis({
      disease: '',
      condition: '',
  ...
  const handleRemoveDiagnosis = (index: number) => {
    const updatedDiagnoses = formData.diagnoses_history.filter((_, i) => i !== index);
    setFormData({ ...formData, diagnoses_history: updatedDiagnoses });
  };
      ...
      <div className="bg-white rounded-2xl shadow-md p-8">
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          <div className="flex flex-col">
            ...
            <input id="id" type="text" name="id" value={formData.id} onChange={handleChange} required className="border border-gray-300 rounded-md px-3 py-2 focus:outline-none focus:ring-2 focus:ring-blue-400 focus:border-blue-400" />
          </div>
          <div className="flex flex-col">
          ...
          <div className="flex flex-col">
            ...
            <input id="age" type="number" name="age"  value={isNaN(formData.age) ? "" : formData.age} onChange={handleChange} required min={1} className="border border-gray-300 rounded-md px-3 py-2 focus:outline-none focus:ring-2 focus:ring-blue-400 focus:border-blue-400" />
          </div>
          <div className="flex flex-col">
          ...
          <h2 className="text-2xl font-semibold mb-4 text-gray-900">Diagnosis History (Optional)</h2>
          ...
          {formData.diagnoses_history.length > 0 && (
            <div className="mt-6">
              ...
              <ul className="list-disc pl-5 space-y-2">
                {formData.diagnoses_history.map((entry, index) => (
